...
class AnalyzeServiceCoverageStrategyAgent(BaseAgent, ProtocolMixin):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        market_opportunity = self._calculate_market_opportunity(market, penetration_analysis, competitors)

        # Prioritize expansion zones
        expansion_priorities = self._prioritize_expansion_zones(zone_performance, coverage_gaps, market)
        ...
        return {
            ...
            "output": {
                ...
                "summary": {
                    ...
                    "top_expansion_zone": expansion_priorities[0]['zone_id'] if expansion_priorities else 'none'
                }
            }
        }
    ...
    def _analyze_market_penetration(
        ...
        user_penetration = (monthly_active_users / covered_population * 100) if covered_population > 0 else 0
        ...
        tam_monthly_users = metro_population * smartphone_penetration * 0.30  # Assume 30% would use ride-sharing
        ...
        return {
            ...
            'market_sizing': {
                ...
                'current_vs_som_percentage': round((monthly_active_users / som_monthly_users * 100), 1) if som_monthly_users > 0 else 0
            },
            'competitive_position': 'leader' if market_share > 35 else 'strong' if market_share > 25 else 'competitive' if market_share > 15 else 'challenger'
        }

    def _identify_coverage_gaps(
        self,
        coverage: Dict,
        zone_performance: List[Dict],
        market: Dict
    ) -> Dict[str, Any]:
        ...
        for zone in zone_performance:
            ...
            if rides_per_capita < avg_rides_per_capita * 0.7:
                underserved_zones.append({
                    ...
                    'gap_percentage': round((1 - rides_per_capita / avg_rides_per_capita) * 100, 1),
        ...
        return {
            ...
            'coverage_quality': 'excellent' if population_coverage >= 90 else 'good' if population_coverage >= 75 else 'fair' if population_coverage >= 60 else 'poor'
        }
    ...
    def _calculate_market_opportunity(
        ...
        rides_per_capita_current = (our_rides / current_coverage_pop) if current_coverage_pop > 0 else 0
        ...
        if competitor_a_rides > our_rides:
            ...
            growth_to_match_leader = (rides_to_match_leader / our_rides * 100) if our_rides > 0 else 0
        else:
        ...
        return {
            ...
            'growth_potential_percentage': round((untapped_rides / our_rides * 100), 1) if our_rides > 0 else 0,
            ...
            'market_maturity': 'emerging' if total_market_rides < 1000000 else 'growing' if total_market_rides < 3000000 else 'mature'
        }
    ...
    def _prioritize_expansion_zones(
        ...
        for zone in coverage_gaps['underserved_zones']:
            ...
            if zone_data:
                ...
                population_score = min(100, (population / 1000) * 2)  # Higher population = higher score
                ...
                potential_additional_rides = population * avg_rides_per_capita * (gap_percentage / 100)
                ...
                expansion_candidates.append({
                    ...
                    'priority': 'high' if expansion_score >= 70 else 'medium' if expansion_score >= 50 else 'low'
                })
    ...
    def _optimize_service_density(
        ...
        density_status = 'optimal' if abs(drivers_per_1000 - target_density_per_1000) < 0.1 else \
                        'high' if drivers_per_1000 > target_density_per_1000 else 'low'
        ...
        return {
            ...
            'recommendation': 'Increase driver count' if density_status == 'low' else 'Redistribute drivers' if density_status == 'high' else 'Maintain current levels'
        }
