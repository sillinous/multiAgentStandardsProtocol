...
class ManageProductionQualityOperationalAgent(BaseAgent, ProtocolMixin):
    ...
    def _defect_rate_analysis(self, defects, total_units):
        ...
        return {
            ...
            "quality_rating": "World Class" if sigma_level >= 6 else "Industry Average" if sigma_level >= 4 else "Needs Improvement"
        }
...
def create_manage_production_quality_operational_agent(config=None):
    return ManageProductionQualityOperationalAgent(config or ManageProductionQualityOperationalAgentConfig())
