...
LIQUIDATION_THRESHOLD = 0.5  # Multiplier for average liquidation to detect significant events
    ...
    def _analyze_opportunity(self, current_longs, current_shorts, previous_longs, previous_shorts):
        ...
        try:
            ...
            market_data = hl.get_data(
                symbol="BTC", timeframe=TIMEFRAME, bars=LOOKBACK_BARS, add_indicators=True
            )
            ...
            if self.deepseek_client and MODEL_OVERRIDE.lower() == "deepseek-chat":
                ...
                response = self.deepseek_client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are a liquidation analyst. You must respond in exactly 3 lines: BUY/SELL/NOTHING, reason, and confidence.",
                        },
            ...
            if current_longs is not None and current_shorts is not None:
                ...
                if not self.liquidation_history.empty:
                    ...
                    if previous_longs > 0 and previous_shorts > 0:
                        ...
                        if current_longs > (previous_longs * threshold) or current_shorts > (
                            previous_shorts * threshold
                        ):
                            # Get AI analysis
