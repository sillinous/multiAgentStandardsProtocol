...
MODEL_OVERRIDE = (
    "0"  # Set to "deepseek-chat" or "deepseek-reasoner" to use DeepSeek, "0" to use default
)
        ...
        try:
            ...
            for _, row in positions.iterrows():
                ...
                if current_value > 0:  # Double check we have a position
                    ...
                    if token_data:
                        position_data[token] = {"value_usd": current_value, "data": token_data}

            if not position_data:
                cprint(
                    "❌ Could not get market data for any monitored positions", "white", "on_red"
                )
                return False
