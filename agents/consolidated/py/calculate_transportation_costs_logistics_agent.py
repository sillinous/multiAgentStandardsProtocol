...
class CalculateTransportationCostsLogisticsAgent(BaseAgent, ProtocolMixin):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        variance_analysis = self._perform_variance_analysis(efficiency_metrics, profitability, budget)

        # Generate cost optimization recommendations
        recommendations = self._generate_cost_recommendations(cost_breakdown, efficiency_metrics, variance_analysis)
    ...
    def _calculate_efficiency_metrics(
        ...
        rides_per_driver_per_day = total_rides / (active_drivers * period_days) if (active_drivers * period_days) > 0 else 0

        # Distance per driver per day
        km_per_driver_per_day = total_distance / (active_drivers * period_days) if (active_drivers * period_days) > 0 else 0
    ...
    def _calculate_profitability(
        ...
        # Operating leverage
        operating_leverage = contribution_margin * total_rides / gross_profit if gross_profit != 0 else 0
        ...
        return {
            ...
            'actual_vs_breakeven_percentage': round((total_rides / breakeven_rides * 100), 1) if breakeven_rides > 0 else 0,
    ...
    def _generate_cost_recommendations(
        ...
        if maintenance_pct > 10:
            recommendations.append({
                ...
                'estimated_annual_savings': round(cost_breakdown['maintenance_costs'] * 0.12 * 12, 2)
            })
        ...
        for variance in variance_analysis.get('variances', []):
            if variance['status'] == 'unfavorable' and abs(variance['variance_percentage']) > 10:
                recommendations.append({
                    ...
                    'potential_savings_percentage': min(abs(variance['variance_percentage']), 20),
                    'target_value': variance['target']
                })

        # Sort by priority and potential savings
        priority_order = {'high': 0, 'medium': 1, 'low': 2}
        recommendations.sort(key=lambda x: (priority_order.get(x['priority'], 3), -x.get('potential_savings_percentage', 0)))
    ...
    def get_input_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                ...
                "cost_inputs": {
                    "type": "object",
                    "description": "Cost parameters and rates"
                },
                "budget": {
                    "type": "object",
                    "description": "Budget targets for variance analysis"
                }
            }
        }
...
def create_calculate_transportation_costs_logistics_agent() -> CalculateTransportationCostsLogisticsAgent:
