...
class ManageVehicleFleetAssetAgent(BaseAgent, ProtocolMixin):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lifecycle_assessment = self._assess_lifecycle(vehicle, usage, standards, depreciation_analysis)
    ...
    def _plan_maintenance(self, vehicle: Dict, usage: Dict, standards: Dict) -> Dict[str, Any]:
        ...
        if current_km >= 60000 and current_km % 30000 < maint_interval:
            service_items.append({'item': 'Transmission service', 'estimated_cost': 150, 'required': False})

        if current_km >= 80000 and current_km % 40000 < maint_interval:
            service_items.append({'item': 'Brake pad replacement', 'estimated_cost': 250, 'required': False})
    ...
    def _calculate_depreciation(self, vehicle: Dict) -> Dict[str, Any]:
        ...
        depreciation_percentage = (total_depreciation / purchase_price * 100) if purchase_price > 0 else 0
    ...
    def _analyze_utilization(self, usage: Dict, standards: Dict) -> Dict[str, Any]:
        ...
        utilization_rate = total_hours / available_hours_per_month if available_hours_per_month > 0 else 0

        # Active utilization (excluding idle time)
        active_utilization = active_hours / available_hours_per_month if available_hours_per_month > 0 else 0
        ...
        performance_status = 'excellent' if utilization_rate >= target_util * 1.1 else \
                           'good' if utilization_rate >= target_util else \
                           'below_target' if utilization_rate >= target_util * 0.8 else 'poor'
    ...
    def _assess_lifecycle(self, vehicle: Dict, usage: Dict, standards: Dict, depreciation: Dict) -> Dict[str, Any]:
    ...
    def _calculate_tco(self, vehicle: Dict, usage: Dict, maintenance: List[Dict], depreciation: Dict) -> Dict[str, Any]:
        """Calculate total cost of ownership"""
        age_years = depreciation['age_years']
        current_km = vehicle.get('current_mileage_km', 0)
    ...
    def _generate_fleet_recommendations(
        ...
        if utilization['performance_status'] == 'poor':
            recommendations.append({
                ...
                'suggestions': ['Increase driver availability', 'Optimize positioning', 'Reduce idle time']
            })
        ...
        if lifecycle['replacement_urgency'] in ['immediate', 'plan']:
            recommendations.append({
                'category': 'lifecycle',
                'priority': 'high' if lifecycle['replacement_urgency'] == 'immediate' else 'medium',
