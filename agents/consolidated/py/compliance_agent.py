...
class ComplianceAgent:
    ...
    def _init_model(self):
        ...
        try:
            cprint(
                f"🤖 Initializing {MODEL_CONFIG['type']} model: {MODEL_CONFIG['name']}...", "cyan"
            )
    ...
    def extract_frames(self, video_path: Path, output_folder: Path) -> List[Path]:
        ...
        try:
            ...
            for frame_number in range(0, frame_count, frame_interval):
                ...
                if saved_count % 10 == 0:
                    cprint(
                        f"💫 Extracted {saved_count} frames... Moon Dev would be proud!", "green"
                    )
            ...
            cprint(
                f"✅ Extraction complete! Saved {saved_count} frames to {output_folder}", "green"
            )
            return frame_paths
        except Exception as e:
        ...
        try:
            ...
            summary_data = {
                ...
                "non_compliant_videos": sum(
                    1 for r in results if not r.get("result", {}).get("is_compliant", False)
                ),
                "video_results": [],
            }
