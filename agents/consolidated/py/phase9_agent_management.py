...
class AgentManagementSystem:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            print(f"Agent Management System initialized with {len(self.agents)} agents from registry")
    ...
    async def update_agent_metrics(
        ...
        agent.metrics.executions_total = metrics.get("executions_total", old_metrics.executions_total)
        agent.metrics.executions_successful = metrics.get("executions_successful", old_metrics.executions_successful)
        agent.metrics.executions_failed = metrics.get("executions_failed", old_metrics.executions_failed)
        agent.metrics.avg_execution_time_ms = metrics.get("avg_execution_time_ms", old_metrics.avg_execution_time_ms)
        agent.metrics.cpu_usage_percent = metrics.get("cpu_usage_percent", old_metrics.cpu_usage_percent)
        ...
        if agent.metrics.executions_total > 0:
            agent.metrics.error_rate = agent.metrics.executions_failed / agent.metrics.executions_total
        else:
    ...
    async def get_dashboard_summary(self) -> Dict[str, Any]:
        ...
        top_agents = sorted(
            agents,
            key=lambda a: a.metrics.executions_successful,
            reverse=True
        )[:5]

        # Agents needing attention
        critical_agents = [a for a in agents if a.health_level in [AgentHealthLevel.CRITICAL, AgentHealthLevel.WARNING]]
    ...
    async def update_agent_version(
        ...
        await self.log_agent_event(
            agent_id,
            "INFO",
            f"Agent updated from v{old_version} to v{new_version}"
        )
    ...
    async def adjust_agent_behavior(
        ...
        await self.log_agent_event(
            agent_id,
            "INFO",
            f"Behavior adjusted: {json.dumps(adjustments)}"
        )
