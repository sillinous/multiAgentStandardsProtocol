...
class OptimizeAssetUtilizationAssetManagementAgent(BaseAgent, ProtocolMixin):
    ...
    def __init__(self, config: OptimizeAssetUtilizationAssetManagementAgentConfig):
        ...
        self.config = config
        self.skills = {'oee_calculation': 0.92, 'capacity_planning': 0.88, 'utilization_analytics': 0.86}
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # OEE Calculation
        oee_analysis = self._calculate_oee(asset_data, downtime_data, production_metrics, quality_metrics)
    ...
    def _calculate_oee(self, assets: List[Dict], downtime: Dict, production: Dict, quality: Dict) -> Dict[str, Any]:
        """
    ...
    def _analyze_capacity_utilization(self, assets: List[Dict], production: Dict) -> Dict[str, Any]:
        ...
        # Calculate averages
        avg_utilization = np.mean([u['utilization_pct'] for u in utilization_by_asset]) if utilization_by_asset else 0
        ...
        for asset in capacity['utilization_by_asset']:
            ...
            # Moderate: Either high utilization OR low OEE
                ...
                moderate_bottlenecks.append({
                    ...
                    "utilization": asset['utilization_pct'],
                    "oee": oee_data['oee'],
                    "issue": "Capacity constraint" if asset['utilization_pct'] > 85 else "Efficiency issue",
                    "impact": "medium"
                })
...
def create_optimize_asset_utilization_asset_management_agent(config: Optional[OptimizeAssetUtilizationAssetManagementAgentConfig] = None):
    if config is None:
