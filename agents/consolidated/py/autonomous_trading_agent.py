...
class AutonomousTradingAgent(AutonomousAgent):
    ...
    def on_start(self):
        ...
        self.register_capability(
            ...
            cost_model={
                'trade_execution': 0.001,
                'position_query': 0.0001,
                'currency': 'SOL'
            }
        )
    ...
    def _handle_trading_signal(self, message):
        ...
        print(f"[{self.agent_id}] 📡 Signal from {source}: {action} {token[:8]}... (confidence: {confidence:.2f})")
    ...
    def _process_sell_signal(self, token: str, confidence: float, source: str):
        ...
        proposal_id = self.propose_consensus(
            ...
            data={
                'token': token,
                'reason': f'Sell signal from {source}',
                'confidence': confidence
            },
            min_votes=2,
            threshold=0.6,
    ...
    def _execute_trade_request(self, token: str, action: str, size_usd: float, requester: str) -> Dict[str, Any]:
