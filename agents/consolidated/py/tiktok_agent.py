...
# Timing settings
CLICK_PAUSE = 0.3  # Pause after movement before clicking
SCROLL_PAUSE = 0.3  # Pause between videos
ACTIVATION_PAUSE = 0.5  # Pause after click to ensure window activation
MOVEMENT_SPEED = 0.5  # Speed of cursor movement (seconds)
BROWSER_LOAD_WAIT = 0.5  # Wait for browser to load TikTok
COMMENT_LOAD_WAIT = 0.5  # Wait for comments to load after clicking
    ...
    try:
            ...
            try:
                ...
                if displays:  # Only print if we actually got display info
                    ...
                    for i, display in enumerate(displays, 1):
                        cprint(
                            f"  ├─ Display {i}: Origin({display['x']}, {display['y']}) Size({display['width']}x{display['height']})",
                            "cyan",
                        )
            except Exception as e:
        ...
        for i in range(steps + 1):
            ...
            if debug and i % 5 == 0:
                ...
                cprint(
                    f"  ├─ Step {i}: Moving to ({int(current_x)}, {int(current_y)}) At: ({int(pos.x)}, {int(pos.y)})",
                    "cyan",
                )
        ...
        if not success:
            cprint(
                f"⚠️ Warning: Expected ({x}, {y}) but got ({int(final_pos.x)}, {int(final_pos.y)})",
                "yellow",
            )
...
def quick_click():
    ...
    try:
        ...
        mouse_down = CG.CGEventCreateMouseEvent(
            None, CG.kCGEventLeftMouseDown, pos, CG.kCGMouseButtonLeft
        )
        mouse_up = CG.CGEventCreateMouseEvent(
            None, CG.kCGEventLeftMouseUp, pos, CG.kCGMouseButtonLeft
        )
...
def capture_screenshot(video_number):
    ...
    for attempt in range(MAX_SCREENSHOT_RETRIES):
        try:
            ...
            cprint(
                f"🎯 Capture region: x={SCREENSHOT_X}, y={SCREENSHOT_Y}, width={SCREENSHOT_WIDTH}, height={SCREENSHOT_HEIGHT}",
                "cyan",
            )
            ...
            if not all(
                ...
                for x in [SCREENSHOT_X, SCREENSHOT_Y, SCREENSHOT_WIDTH, SCREENSHOT_HEIGHT]
            ):
            ...
            cprint(
                f"🌙 Moon Dev says: Alpha secured from TikTok video #{video_number}! 💰", "magenta"
            )
...
def analyze_screenshot(screenshot_path, video_number, video_url=None):
    ...
    try:
        ...
        if not model:
            raise Exception(
                f"Failed to initialize {MODEL_TYPE} model '{MODEL_NAME}'. Check your API keys and model availability."
            )
        ...
        messages = [
            {
                "role": "system",
                "content": "You are an expert at analyzing TikTok videos and comments to extract trading insights.",
            },
            {
                "role": "user",
                "content": [
        ...
        if ANALYSIS_CSV.exists():
            ...
            df = pd.DataFrame(
                columns=["timestamp", "video_number", "screenshot_path", "analysis", "video_url"]
            )
...
def detect_share_button():
    ...
    try:
        ...
        cprint(
            f"📸 Capturing small area around ({SOUND_X}, {SOUND_Y}) - Size: {region_width}x{region_height}",
            "cyan",
        )

        # Create CGImage of specified region
        region = CG.CGRectMake(
            float(region_x), float(region_y), float(region_width), float(region_height)
        )

        # Get screenshot
        cprint("📷 Attempting to capture share/sound button area...", "cyan")
        screenshot = CG.CGWindowListCreateImage(
            region, CG.kCGWindowListOptionOnScreenOnly, CG.kCGNullWindowID, CG.kCGWindowImageDefault
        )
        ...
        messages = [
            {
                "role": "system",
                "content": "You are an expert at analyzing TikTok interface elements. Respond with ONLY 'true' or 'false'.",
            },
            {
                "role": "user",
                "content": [
        ...
        if has_share_button:
            print(
                "\033[42m\033[30m" + "  SHARE BUTTON DETECTED: TRUE  " + "\033[0m"
            )  # Green background with black text
        else:
            print(
                "\033[43m\033[30m" + "  NO SHARE BUTTON DETECTED: FALSE  " + "\033[0m"
        ...
        # Add a fun Moon Dev easter egg message
        cprint(
            f"🌙 Moon Dev says: {'Share button detected! Using alternate comment position.' if has_share_button else 'No share button found. Using standard comment position.'} 🔍",
            "magenta",
        )
        ...
        for attempt in range(3):
            ...
            if attempt % 2 == 0:
                ...
                try:
                    ...
                    cmd_down = CG.CGEventCreateKeyboardEvent(
                        None, 0x37, True
                    )  # 0x37 is Command key
            ...
            try:
                ...
                if (
                    clipboard_content
                    and clipboard_content.startswith("http")
                    and "tiktok.com" in clipboard_content
                ):
                    ...
                    cprint(
                        f"⚠️ Attempt {attempt+1}: Invalid URL or empty clipboard: '{clipboard_content}'",
                        "yellow",
                    )
...
def is_live_video(url=None):
    ...
    try:
        ...
        if is_live:
            print(
                "\033[41m\033[97m"
                + "  🔴 LIVE VIDEO DETECTED! SKIPPING PROCESSING  ".center(56)
                + "\033[0m"
            ...
            print(
                "\033[42m\033[30m"
                + "  ✅ REGULAR VIDEO DETECTED - PROCESSING  ".center(56)
                + "\033[0m"
            )  # Green background with black text
        ...
        # Add a fun Moon Dev easter egg message
        cprint(
            f"🌙 Moon Dev says: {'Live video detected! Skipping processing.' if is_live else 'Regular video detected. Will process this one!'} 🔍",
            "magenta",
        )
...
def safe_navigate_from_live(video_number):
    ...
    try:
        ...
        cprint(
            f"🖱️ Moving to safe area for live videos ({LIVE_SAFE_CLICK_X}, {LIVE_SAFE_CLICK_Y})",
            "cyan",
            attrs=["bold"],
        )
        ...
        cprint(
            f"🌙 Moon Dev says: Safely navigated away from live video #{video_number}! 🚀",
            "magenta",
        )
        return True
...
def scrape_tiktok():
    ...
    try:
        ...
        SHARE_DETECTION_DIR.mkdir(parents=True, exist_ok=True)
        cprint(
            f"\n📁 Created screenshot directories: {SCREENSHOT_DIR} and {SHARE_DETECTION_DIR}",
            "green",
        )
        ...
        cprint(
            f"\n🎯 Using comment button coordinates: ({COMMENT_BUTTON_X}, {comment_y}) for all videos",
            "cyan",
        )
            ...
            try:
                ...
                if is_live:
                    ...
                    if not safe_navigate_from_live(video_number):
                        cprint(
                            "⚠️ Failed to safely navigate from live video, trying fallback method...",
                            "yellow",
                        )
                        ...
                        try:
                            ...
                            esc_key_down = CG.CGEventCreateKeyboardEvent(
                                None, 0x35, True
                            )  # 0x35 is Escape key
                            esc_key_up = CG.CGEventCreateKeyboardEvent(None, 0x35, False)
                            CG.CGEventPost(CG.kCGHIDEventTap, esc_key_down)
                    ...
                    if check_url and "tiktok.com" in check_url:
                        ...
                        cprint(
                            "⚠️ May have navigated away from TikTok - attempting to return", "yellow"
                        )
                ...
                if not screenshot_path:
                    cprint(
                        f"⚠️ Failed to capture screenshot for video #{video_number}, continuing to next video...",
                        "yellow",
                    )
                    continue
                ...
                if not analysis:
                    cprint(
                        f"⚠️ Failed to analyze screenshot for video #{video_number}, but continuing...",
                        "yellow",
                    )
                ...
                if video_number == 1:
                    cprint(
                        f"\n🖱️ Moving to browser area ({BROWSER_X}, {BROWSER_Y}) to activate it",
                        "cyan",
                    )
...
def find_coordinates():
    ...
    try:
        ...
        cprint(
            "Move your mouse to the desired position and press Ctrl+C to capture coordinates",
            "yellow",
        )
