...
class ManageTransportationOperationalAgent(BaseAgent, ProtocolMixin):
    ...
    def _optimize_routes(self, shipments, routes):
        ...
        for route_id, route_shipments in self._group_shipments_by_route(shipments).items():
            ...
            for i in range(len(route_shipments) - 1):
                dist = (
                    abs(
                        route_shipments[i + 1].get("delivery_sequence", 0)
                        - route_shipments[i].get("delivery_sequence", 0)
                    )
                    * 10
                )
                total_distance += dist
    ...
    def _select_carrier(self, carriers, shipment_requirements):
        ...
        for carrier in carriers:
            ...
            coverage_score = (
                100
                if carrier.get("coverage_area") == "national"
                else 75 if carrier.get("coverage_area") == "regional" else 50
            )

            composite_score = (
                (cost_score * 0.4) + (reliability_score * 0.4) + (coverage_score * 0.2)
            )
...
def create_manage_transportation_operational_agent(config=None):
    return ManageTransportationOperationalAgent(
        config or ManageTransportationOperationalAgentConfig()
    )
