...
class AutonomousRiskAgent(AutonomousAgent):
    ...
    def on_start(self):
        ...
        self.register_tool(
            ...
            parameters={
                'type': 'object',
                'properties': {
                    'limit_type': {'type': 'string', 'enum': ['max_loss', 'max_gain', 'min_balance']},
    ...
    def _initialize_portfolio(self):
        ...
        try:
            ...
            # Publish event
            self.publish_event('portfolio_initialized', {
    ...
    def _handle_trade_request(self, message):
        ...
        print(f"[{self.agent_id}] 📋 Trade request from {requester}: {action} {size_usd} USD of {token[:8]}...")
    ...
    def _perform_risk_check(self):
        ...
        try:
            ...
            # Broadcast status
            if risk_score > 0.7:
                self.broadcast('risk_status_update', {
                    'risk_score': risk_score,
                    'exposure': exposure,
                    'level': 'high'
                }, priority=MessagePriority.HIGH)
    ...
    def _handle_limit_breach(self, limit_type: str, value: float):
        ...
        self.broadcast('risk_limit_breach', {
