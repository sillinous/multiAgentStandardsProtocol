...
class AgentMemoryManager:
    ...
    def store_memory(self, agent_name: str, category: str, content: Dict,
                    accessible_by: Optional[List[str]] = None) -> AgentMemory:
        """Store memory entry"""
        memory = AgentMemory(
    ...
    def get_memory(self, agent_name: str, category: Optional[str] = None,
...
class AgentLearningManager:
    ...
    def record_learning(self, agent_name: str, category: str, content: Dict,
                       confidence: float, applicable_to: Optional[List[str]] = None) -> AgentLearning:
    ...
    def suggest_improvements(self, agent_name: str) -> Dict[str, Any]:
            ...
            suggestions['failure_patterns'] = [
                {
                    ...
                    'occurrences': len([x for x in failures if x.content.get('pattern') == f.content.get('pattern')]),
            ...
            suggestions['success_patterns'] = [
                {
                    ...
                    'occurrences': len([x for x in successes if x.content.get('pattern') == s.content.get('pattern')]),
        ...
        if failures:
            suggestions['recommended_changes'] = [
                f"Address failure pattern: {f.content.get('pattern')}"
                for f in failures[:3]
            ]
...
class AgentOutputManager:
    ...
    def store_output(self, agent_name: str, execution_id: str, status: AgentStatus,
                    output_data: Dict, errors: Optional[List[str]] = None,
                    duration_seconds: float = 0.0) -> AgentOutput:
    ...
    def get_latest_outputs(self, agent_name: Optional[str] = None, limit: int = 20) -> List[AgentOutput]:
        """Get latest outputs"""
        outputs = []
        pattern = f"{agent_name}_*.json" if agent_name else "*.json"
