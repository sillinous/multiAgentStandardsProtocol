...
class PerformBudgetingFinancialAgent(BaseAgent, ProtocolMixin):
    ...
    def _allocate_budget(self, total_budget, cost_centers, strategic_priorities):
        ...
        for cost_center in cost_centers:
            ...
            base_allocation = (cost_center.get('priority_weight', 1.0) / total_weight) * total_budget
    ...
    def _zero_based_budgeting_scoring(self, budget_requests):
        ...
        for request in budget_requests:
            ...
            # Composite score
            composite_score = (business_impact * 0.4) + (strategic_alignment * 0.4) + (min(roi / 10, 10) * 0.2)
            
            scored_requests.append({
                "request_id": request.get('request_id'),
                "amount": request.get('amount', 0),
                "composite_score": round(composite_score, 2),
                "recommendation": "Approve" if composite_score >= 7 else "Review" if composite_score >= 5 else "Reject"
            })
