...
class PerformProfitabilityAnalysisFinancialAgent(BaseAgent, ProtocolMixin):
    ...
    def _calculate_contribution_margin(self, revenue_data, variable_costs):
        ...
        for product_id in set(r.get('product_id') for r in revenue_data):
            ...
            contribution_margin_pct = (contribution / total_revenue * 100) if total_revenue > 0 else 0
    ...
    def _profit_waterfall(self, gross_revenue, cost_components):
        ...
        waterfall = [{"stage": "Gross Revenue", "value": gross_revenue, "cumulative": gross_revenue}]
    ...
    def _segment_profitability(self, segments, revenue_data, cost_data):
        ...
        for segment in segments:
            ...
            seg_revenue = sum(r.get('amount', 0) for r in revenue_data if r.get('segment_id') == segment_id)
            seg_costs = sum(c.get('amount', 0) for c in cost_data if c.get('segment_id') == segment_id)
...
def create_perform_profitability_analysis_financial_agent(config=None):
    return PerformProfitabilityAnalysisFinancialAgent(config or PerformProfitabilityAnalysisFinancialAgentConfig())
