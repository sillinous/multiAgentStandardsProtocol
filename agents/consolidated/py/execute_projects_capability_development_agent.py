...
class ExecuteProjectsCapabilityDevelopmentAgent(BaseAgent, ProtocolMixin):
    ...
    def _calculate_critical_path(
        ...
        for task in reversed(tasks):
            ...
            # Find successors
            successors = [
                dep["successor"] for dep in dependencies if dep.get("predecessor") == task_id
            ]
            ...
            if not successors:
                ...
                late_finish[task_id] = min(
                    late_start.get(succ, project_duration) for succ in successors
                )
        ...
        return {
            ...
            "total_slack_buffer": sum(
                late_start.get(t.get("task_id"), 0) - early_start.get(t.get("task_id"), 0)
                for t in tasks
            ),
        }

    def _allocate_resources(
        self, tasks: List[Dict], resources: List[Dict], critical_path: Dict
    ) -> Dict[str, Any]:
        """
        ...
        sorted_tasks = sorted(
            tasks, key=lambda t: 0 if t.get("task_id") in critical_task_ids else 1
        )
        ...
        for task in sorted_tasks:
            ...
            for resource in resources:
                ...
                match_score = (
                    len(set(required_skills) & set(resource_skills)) / len(required_skills)
                    if required_skills
                    else 1
                )
    ...
    def _calculate_earned_value(self, tasks: List[Dict], budget: float) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": (
                ...
                else "on_track" if spi >= 0.9 and cpi >= 0.9 else "at_risk"
            ),
        }

    def _identify_project_risks(
        self, critical_path: Dict, resource_plan: Dict, evm: Dict
    ) -> List[Dict]:
        """
    ...
    def _define_milestones(self, tasks: List[Dict], critical_path: Dict) -> List[Dict]:
        ...
        # Project start
        milestones.append({"name": "Project Kickoff", "date_offset": 0, "type": "start"})
        ...
        for task in critical_path["critical_tasks"]:
            ...
            if (
                cumulative_duration % 30 == 0 or task == critical_path["critical_tasks"][-1]
            ):  # Every 30 days or last task
                milestones.append(
                    {
...
def create_execute_projects_capability_development_agent(
    config: Optional[ExecuteProjectsCapabilityDevelopmentAgentConfig] = None,
):
    if config is None:
