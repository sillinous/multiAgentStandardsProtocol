...
class ManageProductLifecycleSalesMarketingAgent(BaseAgent, ProtocolMixin):
    ...
    def __init__(self, config: ManageProductLifecycleSalesMarketingAgentConfig):
        ...
        self.config = config
        self.skills = {'lifecycle_analysis': 0.90, 's_curve_modeling': 0.87, 'maturity_scoring': 0.86, 'sunset_planning': 0.84}
        ...
        try:
            ...
            return {
                ...
                "output": {
                    ...
                    "recommendations": self._generate_lifecycle_recommendations(lifecycle_stages, maturity_scores)
                }
            }
    ...
    def _analyze_s_curve(self, product_sales: List[Dict]) -> Dict[str, Any]:
        ...
        for product_id in set(sale.get('product_id') for sale in product_sales):
            ...
            if len(cumulative_sales) > 1:
                growth_rate = (cumulative_sales[-1] - cumulative_sales[0]) / cumulative_sales[0] if cumulative_sales[0] > 0 else 0
                
                # Detect inflection point (where growth accelerates/decelerates)
                growth_deltas = [cumulative_sales[i] - cumulative_sales[i-1] for i in range(1, len(cumulative_sales))]
                ...
                product_curves[product_id] = {
                    ...
                    "current_position": "pre_inflection" if len(cumulative_sales) - 1 < max_growth_idx else "post_inflection"
                }
        
        return product_curves

    def _detect_lifecycle_stage(self, product_sales: List[Dict], s_curve_results: Dict) -> Dict[str, Dict]:
        """
        ...
        for product_id in set(sale.get('product_id') for sale in product_sales):
            ...
            if len(recent_quantities) >= 2:
                recent_growth = (recent_quantities[-1] - recent_quantities[0]) / recent_quantities[0] if recent_quantities[0] > 0 else 0
            else:
                recent_growth = 0
    ...
    def _calculate_maturity_scores(self, product_sales: List[Dict], market_data: Dict) -> Dict[str, float]:
        """
        ...
        for product_id in set(sale.get('product_id') for sale in product_sales):
            ...
            if len(sales_data) >= 4:
                ...
                historical_velocity = sum(s.get('quantity', 0) for s in sales_data[:-3]) / max(len(sales_data) - 3, 1)
                velocity_ratio = min(historical_velocity / recent_velocity, 2.0) if recent_velocity > 0 else 2.0
            ...
            penetration = (product_sales_total / total_market_size) * 100 if total_market_size > 0 else 0
    ...
    def _analyze_cannibalization(self, product_sales: List[Dict]) -> Dict[str, Any]:
        ...
        for i, product_a in enumerate(product_ids):
            for product_b in product_ids[i+1:]:
                sales_a = sorted([s for s in product_sales if s.get('product_id') == product_a], 
                               key=lambda x: x.get('date', ''))
                sales_b = sorted([s for s in product_sales if s.get('product_id') == product_b], 
                               key=lambda x: x.get('date', ''))
    ...
    def _identify_sunset_candidates(self, lifecycle_stages: Dict, maturity_scores: Dict) -> List[Dict]:
        ...
        for product_id, stage_info in lifecycle_stages.items():
            ...
            if (stage_info['stage'] == 'Decline' and 
                maturity > 70 and 
                stage_info['recent_growth_rate'] < -0.15):
    ...
    def _generate_lifecycle_recommendations(self, lifecycle_stages: Dict, maturity_scores: Dict) -> List[Dict]:
    ...
    async def health_check(self) -> Dict[str, Any]:
        return {
            ...
            "compliance": {"standardized": True, "interoperable": True, "redeployable": True, 
                          "reusable": True, "atomic": True, "composable": True, 
                          "orchestratable": True, "vendor_agnostic": True}
        }
...
def create_manage_product_lifecycle_sales_marketing_agent(config: Optional[ManageProductLifecycleSalesMarketingAgentConfig] = None):
    if config is None:
