...
class VoiceRecorder:
    ...
    def start_recording(self):
        ...
        try:
            ...
            for i, device in enumerate(devices):
                ...
                if device["max_input_channels"] > 0:
                    if default_input is None:
                        default_input = i
                    if (
                        "default" in device["name"].lower()
                        or "microphone" in device["name"].lower()
                    ):
                        default_input = i
            ...
            cprint(
                "❌ Could not access microphone. Please allow microphone access in your browser.",
                "red",
            )
    ...
    def audio_callback(self, indata, frames, time, status):
        ...
        if self.debug_count < 100 or random.random() < 0.01:
            cprint(
                f"🎤 Volume: {volume_norm:.4f} (max: {max_volume:.4f}, threshold: {VOLUME_THRESHOLD})",
                "cyan",
            )
            if volume_norm == 0:
        ...
        if volume_norm > VOLUME_THRESHOLD or max_volume > VOLUME_THRESHOLD:
            if not self.recording_start:
                cprint(
                    f"🎙️ Speech detected! Volume: {volume_norm:.4f} (max: {max_volume:.4f})", "green"
                )
            ...
            if self.recording_start:
                ...
                if self.silence_start and current_time - self.silence_start <= BREATH_PAUSE:
                    return  # Continue recording through brief pauses

                # If silence long enough and we have enough audio, process it
                if (
                    self.silence_start
                    and current_time - self.silence_start > SPEECH_END_PAUSE
                    and current_time - self.recording_start > MIN_PHRASE_LENGTH
                ):

                    # Check if this might be part of a continuing thought
                    if (
                        self.last_speech_end
                        and current_time - self.last_speech_end < MAX_PHRASE_GAP
                    ):
...
def split_into_phrases(text):
    ...
    for word in words:
        ...
        if (
            any(char in word for char in ".!?,") or len(current) >= 8
        ):  # Increased from 3 to 8 for more natural phrases
...
def log_unknown_question(question):
    ...
    try:
        ...
        new_data = {"timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "question": question}
...
async def test_conversation():
    ...
    try:
        ...
        conversation_history = [
            {
                "role": "system",
                "content": f"""You are Moon Dev's friendly AI assistant. Keep responses very short and concise (1-2 sentences max). Add emojis to make responses fun and engaging.
                ...
                if transcript.strip():
                    ...
                    if not can_answer:
                        ...
                        conversation_history.append(
                            {"role": "assistant", "content": unknown_response}
                        )
                        continue
                    ...
                    try:
                        ...
                        if lang != "en":
                            ...
                            await play_audio_response(
                                "I can only understand English. Please speak in English."
                            )
                            cprint("\n🎤 Listening...", "cyan")
                            continue
                    except:
        ...
        try:
            ...
            response = openai.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are Moon Dev's friendly AI assistant. Keep responses concise and clear for phone calls.",
                    },
