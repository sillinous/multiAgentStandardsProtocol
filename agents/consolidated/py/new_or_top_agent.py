...
def print_fancy(
    message: str, color: str = "white", bg_color: str = "on_blue", emojis: List[str] = None
):
...
class NewOrTopAgent:
    ...
    def __init__(self):
        self.headers = {"x-cg-pro-api-key": COINGECKO_API_KEY, "Content-Type": "application/json"}

        # Initialize AI client based on model
        if "deepseek" in AI_MODEL.lower():
            deepseek_key = os.getenv("DEEPSEEK_KEY")
            if deepseek_key:
                self.ai_client = openai.OpenAI(api_key=deepseek_key, base_url=DEEPSEEK_BASE_URL)
    ...
    def get_top_gainers(self) -> pd.DataFrame:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print_fancy(
                    f"Error fetching top gainers: {response.text}", "white", "on_red", ERROR_EMOJIS
                )
                return pd.DataFrame()
    ...
    def get_new_coins(self) -> pd.DataFrame:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print_fancy(
                    f"Discovered {len(df)} new cosmic tokens! 🪐", "cyan", "on_grey", SUCCESS_EMOJIS
                )
                return df

            else:
                print_fancy(
                    f"Error fetching new coins: {response.text}", "white", "on_red", ERROR_EMOJIS
                )
                return pd.DataFrame()
            ...
            if response.status_code == 200 and ohlcv_response.status_code == 200:
                ...
                if ohlcv_data and len(ohlcv_data) > 0:
                    ohlcv_df = pd.DataFrame(
                        ohlcv_data, columns=["timestamp", "open", "high", "low", "close"]
                    )
                ...
                print_fancy(
                    f"Error fetching coin data: {response.text}", "white", "on_red", ERROR_EMOJIS
                )
                return {}
    ...
    def analyze_coin(self, coin_data: Dict, source_type: str) -> str:
        ...
        try:
            ...
            price_str = (
                f"${coin_data.get('market_data', {}).get('current_price', {}).get('usd', 0):,.8f}"
            )
            ...
            if recommendation == "BUY":
                print_fancy(
                    f"RECOMMENDATION FOR {name} ({symbol}):", "white", "on_green", ["💰", "🚀"]
                )
                print_fancy(
                    f"BUY @ {price_str} ({change_str})", "white", "on_green", ["💰", "🚀", "📈"]
                )
            elif recommendation == "SELL":
                print_fancy(
                    f"RECOMMENDATION FOR {name} ({symbol}):", "white", "on_red", ["💸", "📉"]
                )
                print_fancy(
                    f"SELL @ {price_str} ({change_str})", "white", "on_red", ["💸", "🔻", "📉"]
                )
            else:
                print_fancy(
                    f"RECOMMENDATION FOR {name} ({symbol}):", "white", "on_blue", ["🎯", "⏳"]
                )
                print_fancy(
    ...
    def save_analysis(self, result: Dict):
        ...
        if result["recommendation"] == "BUY":
            ...
            print_fancy(
                f"💰 Added {result['name']} to AI Buys!", "white", "on_green", ["💰", "🚀", "📈"]
            )
        else:
            print_fancy(
                f"💾 Saved analysis for {result['name']}", "white", "on_green", SUCCESS_EMOJIS
            )

    def run_analysis(self):
        """Run complete analysis cycle"""
        print_spinner(
            "🚀 Initiating Moon Dev Analysis Sequence...", ROCKET_SEQUENCE, "white", "on_magenta"
        )
