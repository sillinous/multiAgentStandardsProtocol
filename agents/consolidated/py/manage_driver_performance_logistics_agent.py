...
class ManageDriverPerformanceLogisticsAgent(BaseAgent, ProtocolMixin):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        incentive_calculation = self._calculate_incentives(performance_score, metrics, incentive_config)

        # Generate performance ranking
        ranking_info = self._generate_ranking_info(performance_score)

        # Generate recommendations
        recommendations = self._generate_recommendations(performance_score, behavioral_analysis, quality_metrics)
        ...
        return {
            ...
            "output": {
                ...
                "summary": {
                    ...
                    "improvement_priority": recommendations[0]['category'] if recommendations else 'none'
                }
            }
        }
    ...
    def _analyze_behavioral_patterns(
        ...
        speeding_rate = (behavioral.get('speeding_incidents', 0) / total_rides * 100) if total_rides > 0 else 0
        harsh_braking_rate = (behavioral.get('harsh_braking_events', 0) / total_rides * 100) if total_rides > 0 else 0
        harsh_accel_rate = (behavioral.get('harsh_acceleration_events', 0) / total_rides * 100) if total_rides > 0 else 0
        ...
        primary_concern = max(incident_types.items(), key=lambda x: x[1])[0] if total_incidents > 0 else 'none'
    ...
    def _calculate_incentives(
        ...
        return {
            ...
            'earnings_increase_percentage': round(((adjusted_earnings - base_earnings) / base_earnings * 100), 1) if base_earnings > 0 else 0
        }
    ...
    def _generate_recommendations(
        ...
        if component_scores['customer_rating'] < 85:
            recommendations.append({
                ...
                'recommendation': 'Complete customer service training and focus on ' + ', '.join(quality.get('top_weaknesses', [])[:2]),
