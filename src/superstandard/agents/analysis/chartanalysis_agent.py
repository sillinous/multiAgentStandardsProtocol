...
class ChartAnalysisAgent(BaseAgent):
    ...
    def _analyze_chart(self, symbol, timeframe, data):
        ...
        try:
            ...
            context = CHART_ANALYSIS_PROMPT.format(
                symbol=symbol, timeframe=timeframe, chart_data=chart_data
            )
    ...
    def _format_announcement(self, symbol, timeframe, analysis):
        ...
        try:
            ...
            friendly_timeframe = (
                timeframe.replace("m", " minute").replace("h", " hour").replace("d", " day")
            )
    ...
    def analyze_symbol(self, symbol, timeframe):
        ...
        try:
            # Get market data
            data = hl.get_data(
                symbol=symbol, timeframe=timeframe, bars=LOOKBACK_BARS, add_indicators=True
            )
            ...
            for idx, row in last_5.iterrows():
                ...
                print(
                    f"║ {time_str} │ {row['open']:.2f} │ {row['high']:.2f} │ {row['low']:.2f} │ {row['close']:.2f} │ {row['volume']:.0f} │"
                )
            ...
            if analysis and all(
                k in analysis for k in ["direction", "analysis", "action", "confidence"]
            ):
                # Format and announce
