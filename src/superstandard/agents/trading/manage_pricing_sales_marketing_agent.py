...
class ManagePricingSalesMarketingAgent(BaseAgent, ProtocolMixin):
    ...
    def _analyze_price_elasticity(
        self, elasticity: float, market_conditions: Dict
    ) -> Dict[str, Any]:
        """
            ...
            demand_impacts.append(
                {
                    ...
                    "net_effect": (
                        "positive" if (price_change * (1 + demand_change / 100)) > 0 else "negative"
                    ),
                }
            )
    ...
    def _analyze_competitive_pricing(self, costs: Dict, competitor_prices: Dict) -> Dict[str, Any]:
        ...
        if not competitor_price_list:
            return {"market_position": "no_competitors", "price_gap": 0, "competitive_index": 100}
        ...
        return {
            ...
            "cost_to_market_ratio": (
                round((unit_cost / avg_competitor_price * 100), 2)
    ...
    def _optimize_pricing(self, costs: Dict, elasticity: Dict, competitive: Dict) -> Dict[str, Any]:
        ...
        price_change_pct = (
            ((optimal_price - market_price) / market_price * 100) if market_price > 0 else 0
        )
        ...
        recommendations = [
            {
                ...
                "margin": round(
                    ((optimal_price * 0.95 - unit_cost) / (optimal_price * 0.95) * 100), 2
                ),
            },
            {
                "scenario": "Balanced",
            ...
            },
            {
                ...
                "margin": round(
                    ((optimal_price * 1.1 - unit_cost) / (optimal_price * 1.1) * 100), 2
                ),
            },
        ]
...
def create_manage_pricing_sales_marketing_agent(
    config: Optional[ManagePricingSalesMarketingAgentConfig] = None,
):
    if config is None:
