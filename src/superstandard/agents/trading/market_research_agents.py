# 🔍 Market Research Specialized Agents
...
class BaseMarketResearchAgent(ABC):
    ...
    def __init__(
        self, agent_id: str, agent_name: str, capabilities: List[MarketResearchCapability]
    ):
    ...
    async def initialize(
        self, ai_service: MarketAnalysisAI, blockchain_protocol: BlockchainAgenticProtocol
    ):
        ...
        logger.info(f"✅ {self.agent_name} initialized and ready")
    ...
    async def generate_insight(self, analysis_type: str, data: Dict[str, Any]) -> MarketInsight:
        ...
        try:
            ...
            logger.error(f"❌ {self.agent_name} insight generation failed: {e}")
    ...
    async def identify_opportunity(
        ...
        try:
            ...
            if opportunity_indicators["opportunity_score"] > 0.7:
                ...
                opportunity = await self._generate_business_opportunity(
                    market_data, opportunity_indicators
                )
                ...
                if opportunity.confidence_score > 0.6:
                    ...
                    logger.info(
                        f"🎯 {self.agent_name} identified opportunity: {opportunity.opportunity_name}"
                    )
            ...
            logger.error(f"❌ {self.agent_name} opportunity identification failed: {e}")
            return None
    ...
    async def learn_from_feedback(self, feedback: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"📚 {self.agent_name} learned from feedback")

        except Exception as e:
            logger.error(f"❌ {self.agent_name} learning failed: {e}")
    ...
    def _extract_insight_from_response(
        self, ai_response: Dict[str, Any], analysis_type: str, data: Dict[str, Any]
    ) -> MarketInsight:
    ...
    async def _generate_business_opportunity(
    ...
    async def _reward_opportunity_identification(self):
        if self.blockchain_protocol:
            await self.blockchain_protocol.reward_performance(
                self.agent_id, {"achievement": "opportunity_identification", "value": "high"}
            )
...
class ProductDiscoveryAgent(BaseMarketResearchAgent):
    ...
    async def collaborate(self, other_agent_id: str, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if task.get("type") == "product_trend_correlation":
                ...
                product_data = await self._prepare_product_data_for_correlation(
                    task.get("products", [])
                )
    ...
    async def _analyze_product_market(self, search_terms: List[str]) -> MarketInsight:
        ...
        return self._extract_insight_from_response(
            ai_response,
    ...
    async def _analyze_pricing_patterns(self, products: List[Dict[str, Any]]) -> MarketInsight:
        ...
        insight = MarketInsight(
            ...
            raw_data={
                ...
                "tiers": {"low": len(low_tier), "mid": len(mid_tier), "high": len(high_tier)},
            },
        )
...
class OpportunityIdentificationAgent(BaseMarketResearchAgent):
    ...
    async def _assess_opportunity_risks(self, opportunity_data: Dict[str, Any]) -> MarketInsight:
        ...
        return self._extract_insight_from_response(
            risk_response, "risk_assessment", opportunity_data
        )
...
class MarketResearchAgentOrchestrator:
    ...
    async def initialize(
        ...
        logger.info("🤖 Market Research Agent Orchestrator initialized with all specialized agents")
    ...
    async def analyze_market_opportunity(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            opportunity_insights = await self.agents["opportunity_identification"].analyze(
                opportunity_data
            )
            ...
            logger.info(f"✅ Market opportunity analysis completed: {results['analysis_id']}")
            return results
...
async def initialize_market_research_agents(
    ai_service: MarketAnalysisAI, blockchain_protocol: BlockchainAgenticProtocol
) -> MarketResearchAgentOrchestrator:
    ...
    try:
        ...
        for agent_id, agent in market_research_orchestrator.agents.items():
            ...
            learning_agent = enable_agent_learning(
                agent=agent, agent_name=agent_id, learning_mode=LearningMode.ACTIVE
            )

            # Register in the global learning registry
            register_learning_agent(agent_id, learning_agent)

            logger.info(f"🎓 Enabled learning for agent: {agent_id}")

        logger.info("✨ All market research agents now have learning capabilities enabled")
    except Exception as e:
        logger.warning(f"Failed to enable learning for agents (non-critical): {e}")

    logger.info("🎯 Market Research Agents ready for autonomous market analysis")
