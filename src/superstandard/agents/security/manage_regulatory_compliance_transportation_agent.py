...
class ManageRegulatoryComplianceTransportationAgent(BaseAgent, ProtocolMixin):
        ...
        return {
            ...
            "output": {
                ...
                "summary": {
                    ...
                    "high_risk_areas": len(
                        [r for r in compliance_risks if r["risk_level"] == "high"]
                    ),
    ...
    def _calculate_compliance_scores(
        self, driver_fleet: Dict, licenses: Dict, insurance: Dict, inspections: Dict
    ) -> Dict[str, Any]:
        """
        ...
        insurance_compliance_rate = (
            (compliant_insurance / total_drivers) if total_drivers > 0 else 0
        )
        ...
        inspection_compliance_rate = (
            (passed_inspections / total_drivers) if total_drivers > 0 else 0
        )
        inspection_score = inspection_compliance_rate * 100

        # Calculate weighted overall score
        weights = {"license": 0.35, "insurance": 0.35, "inspection": 0.30}
    ...
    def _analyze_violations(self, violations: List[Dict], driver_fleet: Dict) -> Dict[str, Any]:
        """
        ...
        most_common = (
            max(violation_by_type.items(), key=lambda x: x[1])[0] if violation_by_type else "none"
        )
    ...
    def _generate_compliance_report(
        self, scores: Dict, licenses: Dict, insurance: Dict, inspections: Dict
    ) -> Dict[str, Any]:
        """
        ...
        license_section = {
            ...
            "status": (
                "compliant"
                if scores["component_scores"]["license_compliance"] >= 90
                else "non_compliant"
            ),
            "details": {
                "valid_licenses": licenses.get("valid_licenses", 0),
        ...
        if licenses.get("expired", 0) > 0:
            license_section["findings"].append(
                f"{licenses.get('expired', 0)} drivers have expired licenses"
            )
        if licenses.get("expiring_within_30_days", 0) > 10:
            license_section["findings"].append(
                f"{licenses.get('expiring_within_30_days', 0)} licenses expiring soon"
            )
        ...
        insurance_section = {
            ...
            "status": (
                "compliant"
                if scores["component_scores"]["insurance_compliance"] >= 90
                else "non_compliant"
            ),
            "details": {
        ...
        if insurance.get("non_compliant", 0) > 0:
            insurance_section["findings"].append(
                f"{insurance.get('non_compliant', 0)} drivers lack proper insurance"
            )
        if insurance.get("expiring_within_30_days", 0) > 20:
            insurance_section["findings"].append(
                f"{insurance.get('expiring_within_30_days', 0)} policies expiring soon"
            )
        ...
        inspection_section = {
            ...
            "status": (
                "compliant"
                if scores["component_scores"]["inspection_compliance"] >= 90
                else "non_compliant"
            ),
            "details": {
                "passed_inspections": inspections.get("passed", 0),
        ...
        if inspections.get("overdue", 0) > 0:
            inspection_section["findings"].append(
                f"{inspections.get('overdue', 0)} vehicles have overdue inspections"
            )
    ...
    def _prepare_audit_materials(
        self, scores: Dict, report: Dict, audit_info: Dict
    ) -> Dict[str, Any]:
        """
        Prepare materials for regulatory audit
        """
        if not audit_info:
            return {"audit_scheduled": False, "readiness_percentage": scores["overall_score"]}
        ...
        required_documents = [
            {
                ...
                "status": (
                    "complete"
                    if scores["component_scores"]["license_compliance"] >= 95
                    else "incomplete"
                ),
            ...
            },
            {
                ...
                "status": (
                    "complete"
                    if scores["component_scores"]["insurance_compliance"] >= 95
                    else "incomplete"
                ),
            ...
            },
            {
                ...
                "status": (
                    "complete"
                    if scores["component_scores"]["inspection_compliance"] >= 95
                    else "incomplete"
                ),
    ...
    def _identify_compliance_risks(
        self, licenses: Dict, insurance: Dict, inspections: Dict, violations: List[Dict]
    ) -> List[Dict[str, Any]]:
        """
        Identify compliance risks
        """
    ...
    def _generate_remediation_actions(
        self, risks: List[Dict], scores: Dict
    ) -> List[Dict[str, Any]]:
        """
...
def create_manage_regulatory_compliance_transportation_agent() -> (
