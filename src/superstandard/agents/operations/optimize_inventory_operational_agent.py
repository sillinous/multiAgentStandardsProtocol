...
class OptimizeInventoryOperationalAgent(BaseAgent, ProtocolMixin):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        reorder_points = self._calculate_reorder_points(
            items, demand_forecast, lead_times, safety_stock
        )
    ...
    def _calculate_eoq(
        self, items: List[Dict], demand: Dict, holding_costs: Dict, ordering_costs: Dict
    ) -> Dict[str, Any]:
        """
    ...
    def _calculate_safety_stock(
        self, items: List[Dict], demand: Dict, lead_times: Dict
    ) -> Dict[str, Any]:
        """
        ...
        for item in items:
            ...
            demand_std_dev = demand_data.get(
                "demand_std_dev", average_demand * 0.2
            )  # Default 20% variability
    ...
    def _perform_abc_analysis(self, items: List[Dict], demand: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "classification_summary": {
                ...
                "a_value_percentage": (
                    round((sum(i["annual_value"] for i in category_a) / total_value * 100), 2)
                    if total_value > 0
                    else 0
                ),
                "b_value_percentage": (
                    round((sum(i["annual_value"] for i in category_b) / total_value * 100), 2)
                    if total_value > 0
                    else 0
                ),
                "c_value_percentage": (
                    round((sum(i["annual_value"] for i in category_c) / total_value * 100), 2)
                    if total_value > 0
                    else 0
                ),
            },
    ...
    def _calculate_reorder_points(
        self, items: List[Dict], demand: Dict, lead_times: Dict, safety_stock: Dict
    ) -> Dict[str, Any]:
        """
        ...
        """
        safety_stock_map = {
            ss["item_id"]: ss["safety_stock"] for ss in safety_stock["safety_stock_by_item"]
        }
...
def create_optimize_inventory_operational_agent(
    config: Optional[OptimizeInventoryOperationalAgentConfig] = None,
):
    if config is None:
