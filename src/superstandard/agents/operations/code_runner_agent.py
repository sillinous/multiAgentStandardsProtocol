...
AI_CHAT_SCREENSHOT_PROMPT = (
    "Does this image have more than 5 emojis in it? Return ONLY true or false."
)
...
def simple_click(double_click=True):
    ...
    try:
        ...
        pos = CG.CGEventGetLocation(CG.CGEventCreate(None))
        cprint(
            f"🖱️ {'Double' if double_click else 'Single'} clicking at: ({int(pos.x)}, {int(pos.y)})",
            "cyan",
        )
...
def capture_composer_screenshot():
    ...
    for attempt in range(MAX_SCREENSHOT_RETRIES):
        try:
            ...
            screenshot_dir = Path(
                "/Users/md/Dropbox/dev/github/moon-dev-ai-agents-for-trading/src/data/code_runner/screenshots"
            )
