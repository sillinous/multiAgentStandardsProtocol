...
class AnalyzeCustomerJourneyCustomerExperienceAgent(BaseAgent, ProtocolMixin):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        satisfaction_analysis = self._analyze_satisfaction_drivers(
            satisfaction, journey_data, profile
        )
        ...
        recommendations = self._generate_journey_recommendations(
            friction_points, churn_prediction, journey_map
        )

        # Calculate customer health score
        health_score = self._calculate_customer_health(
            journey_data, satisfaction, profile, churn_prediction
        )
            ...
            "output": {
                ...
                "summary": {
                    ...
                    "primary_friction_point": (
                        friction_points[0]["stage"] if friction_points else "none"
                    ),
    ...
    def _map_customer_journey(
        self, journey_data: Dict[str, Any], touchpoint_perf: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        stages.append(
            {
                ...
                "friction_indicators": {
                    "poor_arrival_accuracy": touchpoint_perf.get("driver_arrival_accuracy", 0)
                    < 0.85,
        ...
        return {
            ...
            "stages_with_friction": sum(
                1 for s in stages if any(s["friction_indicators"].values())
            ),
        }

    def _analyze_satisfaction_drivers(
        self, satisfaction: Dict[str, Any], journey_data: Dict[str, Any], profile: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "primary_positive_driver": next(
                (d for d in drivers if d["impact"] == "positive"), None
            ),
            "primary_negative_driver": next(
                (d for d in drivers if d["impact"] == "negative"), None
            ),
            "satisfaction_trend": satisfaction.get("last_feedback_sentiment", "neutral"),
        }

    def _predict_churn_risk(
        self, journey_data: Dict[str, Any], satisfaction: Dict[str, Any], profile: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _identify_churn_indicators(
        self, activity_ratio: float, days_since_last: int, nps: int, cancellations: int
    ) -> List[str]:
            ...
            for friction_type, has_friction in stage_frictions.items():
                if has_friction:
                    ...
                    impact_score = self._calculate_friction_impact(
                        stage["stage_name"], friction_type, stage["performance_metrics"]
                    )
    ...
    def _calculate_friction_impact(
        self, stage_name: str, friction_type: str, metrics: Dict
    ) -> float:
    ...
    def _generate_journey_recommendations(
        self, friction_points: List[Dict], churn_prediction: Dict, journey_map: Dict
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_customer_health(
        self, journey_data: Dict, satisfaction: Dict, profile: Dict, churn_pred: Dict
    ) -> Dict[str, Any]:
        """
...
def create_analyze_customer_journey_customer_experience_agent() -> (
