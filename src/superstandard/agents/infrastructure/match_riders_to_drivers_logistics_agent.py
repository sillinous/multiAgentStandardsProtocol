...
class MatchRidersToDriversLogisticsAgent(BaseAgent, ProtocolMixin):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        fairness_metrics = self._calculate_fairness_metrics(
            match_candidates, optimal_match, available_drivers
        )
    ...
    def _calculate_match_scores(
        self, rider: Dict[str, Any], drivers: List[Dict], fairness: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_fairness_score(
        self, driver: Dict[str, Any], all_drivers: List[Dict], fairness_settings: Dict[str, Any]
    ) -> float:
        """
    ...
    def _select_optimal_match(
        self, candidates: List[Dict], rider: Dict, fairness_settings: Dict
    ) -> Dict[str, Any]:
        """
        Select the optimal driver match from candidates
        """
        if not candidates:
            return {"driver_id": None, "status": "no_match_found", "reason": "no_available_drivers"}

        # Filter candidates that meet minimum requirements
        eligible = [c for c in candidates if c["meets_preferences"] and c["within_wait_time"]]
    ...
    def _calculate_alternatives(
        self, candidates: List[Dict], optimal_match: Dict
    ) -> List[Dict[str, Any]]:
        """
        ...
        alternatives = [c for c in candidates if c["driver_id"] != optimal_match.get("driver_id")][
            :3
        ]
    ...
    def _calculate_fairness_metrics(
        self, candidates: List[Dict], optimal_match: Dict, all_drivers: List[Dict]
    ) -> Dict[str, Any]:
        """
        Calculate fairness metrics for the matching decision
        """
        if not optimal_match.get("driver_id"):
            return {"fairness_score": 0, "earnings_balance_score": 0, "ride_distribution_score": 0}
        ...
        earnings_percentile = (
            sum(1 for e in earnings_list if e <= matched_earnings) / len(earnings_list) * 100
        )
    ...
    def _generate_match_explanation(self, optimal_match: Dict, rider: Dict) -> Dict[str, Any]:
        """
        ...
        if optimal_match["component_scores"]["distance"] >= 80:
            ...
            reasons.append(
                f"Driver is reasonably close ({optimal_match['distance_km']:.1f} km away)"
            )
        ...
        return {
            ...
            "match_quality": (
                "excellent"
                if optimal_match["overall_score"] >= 85
                else "good" if optimal_match["overall_score"] >= 70 else "fair"
            ),
