...
class JourneyDocumentationAgent:
    ...
    def generate_status_update(self) -> Dict:
        ...
        return {
            ...
            "recent_progress": [
                {"title": e["title"], "category": e["category"], "outcome": e["outcome"]}
                for e in recent_events
                if e["category"] in ["milestone", "decision"]
            ],
            "active_blockers": self.current_state.get("blockers", []),
            "key_decisions": [
                {"decision": d["decision"], "rationale": d["rationale"], "when": d["timestamp"]}
                for d in self.decision_log[-5:]  # Last 5 decisions
            ],
            "recent_pivots": [
                {"from": p["from"], "to": p["to"], "why": p["why"]}
    ...
    def _summarize_journey(self) -> List[Dict]:
        ...
        return [
            {"milestone": m["title"], "achieved": m["timestamp"], "impact": m["outcome"]}
            for m in milestones
        ]
    ...
    def _generate_help_wanted(self) -> List[Dict]:
        ...
        for blocker in blockers:
            help_wanted.append(
                {"type": "blocker", "need": blocker, "how_to_help": f"Help solve: {blocker}"}
            )

        for opp in opportunities:
            help_wanted.append(
                {"type": "opportunity", "need": opp, "how_to_help": f"Contribute to: {opp}"}
            )
