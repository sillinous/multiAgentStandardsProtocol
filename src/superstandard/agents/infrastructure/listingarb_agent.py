...
class AIAgent:
    ...
    def __init__(self, name: str, model: str):
        ...
        self.memory = {"analyzed_tokens": [], "promising_tokens": [], "conversations": []}
        self.load_memory()
            ...
            if self.memory_file.exists():
                with open(self.memory_file, "r") as f:
                    try:
                        ...
                        print(
                            f"📚 Loaded {len(self.memory['conversations'])} previous conversations for {self.name}"
                        )
                    except json.JSONDecodeError:
                        print(
                            f"⚠️ Warning: Corrupted memory file for {self.name}, using empty memory"
                        )
            else:
...
Remember to reference specific data points from the OHLCV table in your analysis!"""
            ...
            print(
                f"⚠️ Token data: {token_data.get('name', 'Unknown')} ({token_data.get('symbol', 'Unknown')})"
            )
...
class ListingArbSystem:
    ...
    def load_discovered_tokens(self) -> pd.DataFrame:
        ...
        if not DISCOVERED_TOKENS_FILE.exists():
            raise FileNotFoundError(
                f"❌ No discovered tokens file found at {DISCOVERED_TOKENS_FILE}"
            )
    ...
    def analyze_tokens_parallel(self, tokens_batch):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=PARALLEL_PROCESSES) as executor:
            ...
            if tokens_to_analyze:
                ...
                futures = [
                    executor.submit(self.analyze_token, token) for token in tokens_to_analyze
                ]
    ...
    def analyze_token(self, token_data: Dict):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"📊 Recommendations: Agent One: {agent_one_rec} | Agent Two: {agent_two_rec}"
                )
    ...
    def run_analysis_cycle(self):
        ...
        try:
            ...
            batches = [
                discovered_tokens_df[i : i + batch_size]
                for i in range(0, len(discovered_tokens_df), batch_size)
            ]
