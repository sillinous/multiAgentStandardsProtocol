...
LOOKBACK_PERIODS = {"15min": 15}  # Simplified to just 15 minutes
...
class WhaleAgent(BaseAgent):
    ...
    def _save_oi_data(self, timestamp, btc_oi, eth_oi, total_oi):
        ...
        try:
            ...
            if not self.oi_history.empty:
                ...
                total_change_pct = (
                    (total_oi - prev_data["total_oi"]) / prev_data["total_oi"]
                ) * 100
    ...
    def _calculate_changes(self, current_oi):
        ...
        for idx, row in self.oi_history.iterrows():
            print(
                f"{row['timestamp']} | ${row['btc_oi']:,.2f} | ${row['eth_oi']:,.2f} | ${row['total_oi']:,.2f} | {row['btc_change_pct']:,.4f}% | {row['eth_change_pct']:,.4f}% | {row['total_change_pct']:,.4f}%"
            )
        print("=" * 80)
        ...
        if changes:
            ...
            if is_whale:
                ...
                market_data = hl.get_data(
                    symbol="BTC", timeframe="15m", bars=100, add_indicators=True
                )

            # Build base message
            message = (
                f"ayo moon dev 777! BTC OI {btc_direction} {abs(btc_change):.3f}% in {interval}m, "
            )
    ...
    def _announce_initial_summary(self):
        ...
        try:
            ...
            if not changes:
                ...
                latest_data = self.oi_history.iloc[-1]
                minutes_diff = (
                    latest_data["timestamp"] - earliest_data["timestamp"]
                ).total_seconds() / 60
                pct_change = (
                    (latest_data["total_oi"] - earliest_data["total_oi"])
                    / earliest_data["total_oi"]
                ) * 100
    ...
    def _detect_whale_activity(self, current_change):
        ...
        try:
            ...
            historical_changes = (
                self.oi_history["btc_change_pct"].abs().rolling(window=10).mean().dropna()
            )
            ...
            print(
                f"Threshold ({(WHALE_THRESHOLD_MULTIPLIER-1)*100:.0f}% above avg): {threshold:.4f}%"
            )
