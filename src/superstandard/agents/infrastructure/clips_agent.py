...
INPUT_DIR = Path(
    "/Users/md/Dropbox/dev/github/moon-dev-ai-agents-for-trading/src/data/videos/raw_streams"
)
OUTPUT_DIR = Path(
    "/Users/md/Dropbox/dev/github/moon-dev-ai-agents-for-trading/src/data/videos/finished_clips"
)
TEMP_DIR = Path("/Users/md/Dropbox/dev/github/moon-dev-ai-agents-for-trading/src/data/videos/temp")
YOUTUBE_MATERIALS_DIR = Path(
    "/Users/md/Dropbox/dev/github/moon-dev-ai-agents-for-trading/src/data/videos/youtube_materials"
)
    ...
    def generate_voice_intro(self, text, video_id):
        ...
        try:
            ...
            response = self.openai_client.audio.speech.create(
                model=VOICE_MODEL, voice=VOICE_NAME, speed=VOICE_SPEED, input=text
            )
    ...
    def merge_voice_with_clip(self, voice_path, clip_path, output_path):
        ...
        try:
            ...
            final_video = CompositeVideoClip(
                [intro_with_voice, main_video.set_start(voice_duration)]
            )
                ...
                for video_path in video_files:
                    ...
                    if PROCESS_YOUTUBE:
                        ...
                        if not full_transcript:
                            cprint(
                                "⚠️ Could not get YouTube materials, skipping AI analysis", "yellow"
                            )
                    ...
                    for i, clip in enumerate(clip_boundaries, 1):
                        ...
                        cprint(
                            f"⏱️ Time range: {clip['start_time']:.1f}s to {clip['end_time']:.1f}s",
                            "cyan",
                        )
                        ...
                        if PROCESS_YOUTUBE and full_transcript:
                            ...
                            transcript_segment = self.get_transcript_segment(
                                full_transcript, clip["start_percent"], clip["end_percent"]
                            )

                            if transcript_segment:
                                cprint(
                                    f"📜 Transcript segment length: {len(transcript_segment)} chars",
                                    "cyan",
                                )
                                ...
                                if analysis:
                                    ...
                                    if voice_path:
                                        ...
                                        if self._split_video_ffmpeg(
                                            ...
                                            if self.merge_voice_with_clip(
                                                voice_path, temp_clip_path, output_path
                                            ):
                                                cprint(
                                                    f"✨ Created clip {i} with voice intro: {output_path.name}",
                                                    "green",
                                                )
                                            else:
                                                cprint(
                                                    f"❌ Failed to merge voice with clip {i}", "red"
                                                )
                                        else:
                                            cprint(f"❌ Failed to create initial clip {i}", "red")
                                    else:
                                        cprint(
                                            f"❌ Failed to generate voice intro for clip {i}", "red"
                                        )
        ...
        try:
            ...
            if MODEL_TYPE == "ollama":
                ...
                if response:
                    ...
                    sentences = [
                        s.strip() for s in content.replace('"', "").split(".") if s.strip()
                    ]
                    if len(sentences) > MAX_SENTENCES:
                        cprint(
                            f"\n⚠️ Response too long ({len(sentences)} sentences) - retrying...",
                            "yellow",
                        )
                        max_retries = 3
                        retry_count = 0
                        ...
                        while len(sentences) > MAX_SENTENCES and retry_count < max_retries:
                            ...
                            response = self.model.generate_response(
                                ...
                                user_content=formatted_prompt,
                                temperature=0.7
                                + (retry_count * 0.1),  # Increase temperature slightly each retry
                            )
                        ...
                        if len(sentences) > MAX_SENTENCES:
                            cprint(
                                f"⚠️ Still got {len(sentences)} sentences after {max_retries} retries. Using best attempt.",
                                "yellow",
                            )
