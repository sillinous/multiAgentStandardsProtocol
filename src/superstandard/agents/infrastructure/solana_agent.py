...
import random
from src.nice_funcs import token_overview, token_security_info, token_creation_info, token_price
            ...
            try:
                ...
                if security:
                    ...
                    if top_holders_pct > MAX_TOP_HOLDERS_PCT:
                        print(
                            f"👥 High holder concentration ({top_holders_pct:.1f}%) for {token_address[:8]}"
                        )
                        return None
    ...
    def analyze_tokens(self, df):
        ...
        if "Token Address" in df.columns:
            ...
            df["token_address"] = df["birdeye_link"].apply(
                lambda x: x.split("/token/")[1].split("?")[0] if isinstance(x, str) else None
            )
