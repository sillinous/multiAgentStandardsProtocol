...
class ClassificationAgent(BaseAgent):
    ...
    async def _classify_local(self, text: str, categories: List[str]) -> Dict[str, Any]:
        ...
        return {"category": category, "confidence": 0.85}  # Estimated for local LLM

    async def _classify_cloud(self, text: str, categories: List[str]) -> Dict[str, Any]:
    ...
    async def classify_product(self, product_name: str, quality: str = "fast") -> str:
        """
        Quick product classification (returns just the category).

        Optimized for speed - defaults to local processing.
        """
        result = await self.execute(text=product_name, quality=quality, return_confidence=False)
        return result.get("category", "Other")

    async def classify_with_custom_taxonomy(
        self, text: str, custom_categories: List[str], quality: str = "auto"
    ) -> Dict[str, Any]:
        """
        ...
        """
        return await self.execute(text=text, categories=custom_categories, quality=quality)

    async def batch_classify(
        self, texts: List[str], categories: Optional[List[str]] = None, quality: str = "fast"
    ) -> List[Dict[str, Any]]:
        """
        ...
        for text in texts:
            try:
                result = await self.execute(text=text, categories=categories, quality=quality)
                results.append(result)
            except Exception as e:
                logger.error(f"Batch classification item failed: {e}")
                results.append({"category": "Other", "confidence": 0.0, "error": str(e)})
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            "local_percentage": (
                (self.stats["local_classifications"] / total * 100) if total > 0 else 0
            ),
            "cloud_percentage": (
                (self.stats["cloud_classifications"] / total * 100) if total > 0 else 0
            ),
            "estimated_cost_saved": self.stats["credits_saved"] * 1.0,
            "top_categories": sorted(
                self.stats["category_distribution"].items(), key=lambda x: x[1], reverse=True
            )[:5],
    ...
    def get_capabilities_manifest(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cost": {"local": "FREE (0 credits)", "cloud": "~0.01 credits per classification"},
            "performance": {
                "local_latency": "<1s",
                "cloud_latency": "2-3s",
                "quality_local": "85%",
