...
class ExampleAgent(AutonomousAgent):
    ...
    def on_start(self):
        ...
        self.publish_event(
            "agent_started", {"agent_id": self.agent_id, "timestamp": datetime.now().isoformat()}
        )
    ...
    def on_stop(self):
        ...
        self.publish_event(
            "agent_stopped", {"agent_id": self.agent_id, "timestamp": datetime.now().isoformat()}
        )
    ...
    def _handle_price_update(self, message: Message):
        ...
        if price > threshold:
            self.broadcast(
                "price_alert",
                {"token": token, "price": price, "threshold": threshold, "alert_by": self.agent_id},
    ...
    def _on_vote_request(self, proposal_data: Dict[str, Any]):
        ...
        reasoning = (
            f"Agent {self.agent_id} {'approves' if approve else 'rejects'} based on analysis"
        )

        # Cast vote
        self.vote(
            proposal_id=proposal_id, approve=approve, confidence=confidence, reasoning=reasoning
        )

        print(
            f"[{self.agent_id}] Voted: {'APPROVE' if approve else 'REJECT'} (confidence: {confidence:.2f})"
        )
    ...
    def _execute_action(self, action: str, data: Dict[str, Any]):
        ...
        # Publish event
        self.publish_event(
            "action_executed", {"action": action, "data": data, "executed_by": self.agent_id}
        )
...
def main():
    ...
    try:
        ...
        agents[0].broadcast(
            "market.price_update",
            {"token": "SOL", "price": 102.50, "volume": 1000000},
            priority=MessagePriority.NORMAL,
        )
