...
class SourceCandidatesHumanCapitalAgent(BaseAgent, ProtocolMixin):
    ...
    def __init__(self, config: SourceCandidatesHumanCapitalAgentConfig):
        ...
        self.integration = {
            "compatible_agents": ["7.1", "7.3", "13.0"],
            "required_services": [
                "knowledge_graph",
                "ats_system",
                "event_bus",
    ...
    async def _process_candidate_sourcing(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Process candidate_sourcing task with AI-powered analysis.

        Implements APQC process: Source Candidates
        Domain: human_capital
        ...
        processor = get_processor("human_capital")

        # Prepare context for processing
        processing_context = {
            "apqc_process": "Source Candidates",
        ...
        if not analysis_results:
            analysis_results = {
                ...
                "domain": processing_result.get("domain", "human_capital"),
...
def create_source_candidates_human_capital_agent(
    config: Optional[SourceCandidatesHumanCapitalAgentConfig] = None,
