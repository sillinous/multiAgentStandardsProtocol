...
def list_agents():
    ...
    return jsonify({"total": len(agents), "agents": agents}), 200
...
def get_agents_by_category(category):
    ...
    return jsonify({"category": category, "total": len(agents), "agents": agents}), 200
...
def list_categories():
    ...
    return jsonify({"total": len(categories), "categories": sorted(list(categories))}), 200
...
def get_shared_memory():
    ...
    return jsonify({"total": len(memories), "memories": [m.to_dict() for m in memories]}), 200
...
def store_shared_memory():
    ...
    try:
        ...
        return jsonify({"message": "Memory stored successfully", "memory": memory.to_dict()}), 201
    except Exception as e:
...
def record_learning():
    ...
    try:
        ...
        return jsonify({"message": "Learning recorded", "learning": learning.to_dict()}), 201
    except Exception as e:
        return jsonify({"error": str(e)}), 400
...
def get_agent_outputs(agent_name):
    ...
    return (
        jsonify(
            {"agent": agent_name, "total": len(outputs), "outputs": [o.to_dict() for o in outputs]}
        ),
        200,
    )
...
def store_agent_output():
    ...
    try:
        ...
        return jsonify({"message": "Output stored", "output": output.to_dict()}), 201
    except Exception as e:
        return jsonify({"error": str(e)}), 400
    ...
    for memory in memories:
        if memory.accessible_by:
            ...
            for target in memory.accessible_by:
                key = f"{source} -> {target}"
                interactions["shared_memory_flows"][key] = (
                    interactions["shared_memory_flows"].get(key, 0) + 1
                )
                ...
                with open(learning_file, "r") as f:
                    ...
                    if learning_data.get("applicable_to"):
                        ...
                        for target in learning_data["applicable_to"]:
                            ...
                            interactions["learning_transfers"][key] = (
                                interactions["learning_transfers"].get(key, 0) + 1
                            )
            except:
                pass
