...
class ManageCompensationHumanCapitalAgent(BaseAgent, ProtocolMixin):
    ...
    def _pay_equity_analysis(self, employee_data):
        ...
        for job_level in set(e.get("job_level") for e in employee_data):
            ...
            # Gender pay analysis
            male_avg = np.mean(
                [e.get("salary", 0) for e in level_employees if e.get("gender") == "M"]
            )
            female_avg = np.mean(
                [e.get("salary", 0) for e in level_employees if e.get("gender") == "F"]
            )
    ...
    def _incentive_modeling(self, performance_ratings, budget_constraints):
        ...
        for employee in performance_ratings:
            ...
            performance_weight = (
                (performance_score / total_performance_score) if total_performance_score > 0 else 0
            )
        ...
        for employee_id, comp in internal_comp.items():
            ...
            benchmarking[employee_id] = {
                ...
                "competitive_position": (
                    "Above Market"
                    if market_position > 110
                    else "At Market" if market_position >= 90 else "Below Market"
                ),
            }
...
def create_manage_compensation_human_capital_agent(config=None):
    return ManageCompensationHumanCapitalAgent(
        config or ManageCompensationHumanCapitalAgentConfig()
    )
