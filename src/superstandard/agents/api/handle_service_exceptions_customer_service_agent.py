...
class HandleServiceExceptionsCustomerServiceAgent(BaseAgent, ProtocolMixin):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        resolution_strategy = self._determine_resolution(
            exception, severity_analysis, ride_details, customer_history
        )

        # Calculate compensation
        compensation = self._calculate_compensation(
            exception, ride_details, customer_history, severity_analysis
        )

        # Determine if escalation is needed
        escalation_decision = self._evaluate_escalation(
            exception, severity_analysis, resolution_strategy
        )
        ...
        follow_up_actions = self._generate_follow_up_actions(
            exception, driver_history, resolution_strategy
        )
    ...
    def _determine_resolution(
        ...
        if customer_history.get("lifetime_value", 0) > 5000 and resolution_type in [
            "no_compensation",
            "partial_credit",
        ]:
            resolution_type = "credit_and_apology"
    ...
    def _calculate_compensation(
        ...
        if (
            ...
            and ride_details.get("ride_status") == "cancelled_by_driver"
        ):
            # Full fare as credit
            compensation_amount = fare_amount
            compensation_type = "account_credit"
            compensation_items.append(
                {"item": "Full ride credit", "amount": fare_amount, "reason": "Driver cancellation"}
            )
        ...
        if (
            ...
            or customer_history.get("previous_exceptions", 0) > 2
        ):
            promo_code = {
    ...
    def _evaluate_escalation(
        self, exception: Dict[str, Any], severity: Dict[str, Any], resolution: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_communication(
        self, exception: Dict[str, Any], resolution: Dict[str, Any], compensation: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if exception_type == "cancellation":
            ...
            message_parts.append(
                "Thank you for bringing this matter to our attention. We sincerely apologize for the inconvenience you experienced."
            )
        elif exception_type == "safety_concern":
            message_parts.append(
                "We take safety concerns very seriously and apologize for any distress this incident may have caused."
            )
        ...
        if "refund" in resolution_type:
            if resolution_type == "full_refund":
                message_parts.append(
                    f"We have issued a full refund of ${compensation['total_amount']:.2f} to your original payment method."
                )
            else:
                message_parts.append(
                    f"We have issued a partial refund of ${compensation['total_amount']:.2f} to your original payment method."
                )
        elif "credit" in resolution_type:
            message_parts.append(
                f"We have added ${compensation['total_amount']:.2f} in credit to your account for future rides."
            )
        ...
        if compensation.get("promo_code"):
            ...
            message_parts.append("")
            message_parts.append(
                f"Additionally, please use promo code {promo['code']} for {promo['discount_percentage']}% off your next ride (up to ${promo['max_discount_amount']})."
            )
    ...
    def _generate_follow_up_actions(
        self, exception: Dict[str, Any], driver_history: Dict[str, Any], resolution: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
...
def create_handle_service_exceptions_customer_service_agent() -> (
