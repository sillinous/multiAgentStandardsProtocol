...
class Agent:
        ...
        try:
            ...
            while True:  # let the agent iterate on his thoughts until he stops by using a tool
                ...
                try:
                    ...
                    PrintStyle(
                        bold=True, font_color="green", padding=True, background_color="white"
                    ...
                    if not self.handle_intervention(agent_response):
                        if (
                            ...
                            tools_result = self.process_tools(
                                agent_response
                ...
                # Forward errors to the LLM, maybe he can fix them
                    ...
                    msg_response = files.read_file(
                        "./prompts/fw.error.md", error=error_message
    ...
    def append_message(self, msg: str, human: bool = False):
        ...
        if self.history and self.history[-1].type == message_type:
            ...
            self.cleanup_history(
                self.config.msgs_keep_max, self.config.msgs_keep_start, self.config.msgs_keep_end
            )
        if message_type == "ai":
    ...
    def handle_intervention(self, progress: str = "") -> bool:
        ...
        if (
            self.intervention_message and not self.intervention_status
        ):  # if there is an intervention message, but not yet processed
            if progress.strip():
                self.append_message(progress)  # append the response generated so far
            user_msg = files.read_file(
                "./prompts/fw.intervention.md", user_message=self.intervention_message
