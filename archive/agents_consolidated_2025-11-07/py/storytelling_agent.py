...
class StorytellingAgent:
    ...
    def generate_twitter_thread(self, story_beat: Dict) -> List[str]:
        ...
        # Tweet 1: Hook
        thread.append(
            narrative.get("hook", f"Building in public: {event.get('title', 'New update')}")
        )

        # Tweet 2: Setup/Challenge
        thread.append(
            narrative.get("challenge", narrative.get("setup", "The challenge we faced..."))
        )
        ...
        thread.append(
            narrative.get("solution", narrative.get("result", event.get("impact", "The outcome")))
        )
