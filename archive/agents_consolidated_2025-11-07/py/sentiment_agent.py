...
class SentimentAgent:
    ...
    def init_twitter_client(self):
        ...
        try:
            ...
            cprint(
                "🚀 Moon Dev's cookies loaded successfully! Time to fly to the moon! 🌙", "green"
            )
            return client
    ...
    async def get_tweets(self, query):
        ...
        try:
            cprint(
                f"🕒 Time is {datetime.now()} - Moon Dev getting fresh tweets for {query}! 🌟",
                "cyan",
            )
...
if __name__ == "__main__":
    try:
        ...
        cprint(
            f"\n🌙 Moon Dev's Sentiment Agent starting (checking every {CHECK_INTERVAL_MINUTES} minutes)...",
            "cyan",
        )
