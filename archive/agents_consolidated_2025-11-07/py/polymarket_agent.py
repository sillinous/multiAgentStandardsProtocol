...
class PolymarketAgent:
    ...
    def is_near_resolution(self, price):
        ...
        return price_float <= IGNORE_PRICE_THRESHOLD or price_float >= (
            1.0 - IGNORE_PRICE_THRESHOLD
        )
                ...
                if topic == "activity" and msg_type == "orders_matched":
                    ...
                    if usd_amount >= MIN_TRADE_SIZE_USD and not self.is_near_resolution(price):
                        ...
                        trade_data = {
                            ...
                            "conditionId": payload.get(
                                "conditionId", payload.get("id", f"ws_{time.time()}")
                            ),
    ...
    def fetch_historical_trades(self, hours_back=None):
        ...
        try:
            ...
            params = {"limit": 1000, "_min_timestamp": cutoff_timestamp}  # Max allowed by API
        ...
        for market_id, trade in unique_markets.items():
            try:
                ...
                self.markets_df = pd.concat(
                    [self.markets_df, pd.DataFrame([new_market])], ignore_index=True
                )
...
Provide predictions for each market in the specified format."""
        ...
        if USE_SWARM_MODE and self.swarm:
            ...
            try:
                ...
                successful_responses = [
                    name
                    for name, data in swarm_result.get("responses", {}).items()
                    if data.get("success")
                ]
                ...
                cprint(
                    f"✅ Received {len(successful_responses)} successful responses from swarm",
                    "green",
                )
                ...
                for model_name, model_data in swarm_result.get("responses", {}).items():
                    if model_data.get("success"):
                        ...
                        cprint(
                            f"\n⚠️ {model_name.upper()} failed: {model_data.get('error', 'Unknown error')}",
                            "yellow",
                        )
            ...
            try:
                response = self.model.generate_response(
                    system_prompt=system_prompt, user_content=user_prompt, temperature=0.7
                )
                ...
                self.predictions_df = pd.concat(
                    [self.predictions_df, pd.DataFrame([prediction_record])], ignore_index=True
                )
    ...
    def _save_swarm_predictions(self, analysis_run_id, analysis_timestamp, markets, swarm_result):
        ...
        try:
            ...
            prediction_record = {
                ...
                "num_models_responded": len(
                    [v for v in model_predictions.values() if v != "FAILED"]
                ),
            }
    ...
    def status_display_loop(self):
        ...
        while True:
            try:
                ...
                cprint(
                    f"   New unanalyzed: {new_markets}",
                    "yellow" if new_markets < NEW_MARKETS_FOR_ANALYSIS else "green",
                    attrs=["bold"],
                )

                if new_markets >= NEW_MARKETS_FOR_ANALYSIS:
                    cprint(
                        f"   ✅ Ready for analysis! (Have {new_markets}, need {NEW_MARKETS_FOR_ANALYSIS})",
                        "green",
                        attrs=["bold"],
                    )
                else:
                    cprint(
                        f"   ⏳ Collecting... (Have {new_markets}, need {NEW_MARKETS_FOR_ANALYSIS})",
                        "yellow",
                    )
    ...
    def analysis_cycle(self):
        ...
        cprint(
            f"   New unanalyzed markets: {new_markets}",
            "yellow" if new_markets < NEW_MARKETS_FOR_ANALYSIS else "green",
            attrs=["bold"],
        )
        cprint("", "white")
        ...
        if is_first_run:
            ...
            cprint(
                f"   Future runs will require {NEW_MARKETS_FOR_ANALYSIS} NEW markets\n", "yellow"
            )
        else:
            ...
            if new_markets >= NEW_MARKETS_FOR_ANALYSIS:
                ...
                cprint(
                    f"   ❌ Need {NEW_MARKETS_FOR_ANALYSIS - new_markets} more markets",
                    "yellow",
                    attrs=["bold"],
                )
            cprint("", "white")
        ...
        should_analyze = (is_first_run and total_markets > 0) or (
            new_markets >= NEW_MARKETS_FOR_ANALYSIS
        )
        ...
        if should_analyze:
            ...
            cprint(
                f"\n💾 Updated analysis tracker: {self.last_analyzed_count} markets analyzed",
                "green",
            )
        else:
...
def main():
    ...
    cprint(
        f"🚫 Filtering out crypto/Bitcoin markets ({len(IGNORE_CRYPTO_KEYWORDS)} keywords)", "red"
    )
    cprint(f"🚫 Filtering out sports markets ({len(IGNORE_SPORTS_KEYWORDS)} keywords)", "red")
    cprint(
        f"📜 Lookback period: {LOOKBACK_HOURS} hours (fetches historical data on startup)", "yellow"
    )
    cprint("", "yellow")
