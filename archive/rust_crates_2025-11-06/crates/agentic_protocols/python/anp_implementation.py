...
class AgentNetworkRegistry:
    ...
    async def register_agent(self, registration: ANPRegistration) -> Dict[str, Any]:
        ...
        try:
            ...
            # Emit event
            await self._emit_event("agent_registered" if not is_update else "agent_updated", agent_info)

            logger.info(f"✅ Agent {'updated' if is_update else 'registered'}: {agent_id} ({registration.agent_type})")
    ...
    async def heartbeat(self, agent_id: str, health_status: Optional[str] = None,
                       load_score: Optional[float] = None) -> Dict[str, Any]:
        """
    ...
    async def get_network_topology(self) -> NetworkTopology:
        ...
        # Average load
        if self.agents:
            topology.average_load = sum(a.load_score for a in self.agents.values()) / len(self.agents)
...
class ANPClient:
    ...
    async def register(self, agent_id: str, agent_type: str,
                      capabilities: List[str],
    ...
    async def discover(self, capabilities: Optional[List[str]] = None,
                      agent_type: Optional[str] = None,
                      health_status: Optional[str] = None,
