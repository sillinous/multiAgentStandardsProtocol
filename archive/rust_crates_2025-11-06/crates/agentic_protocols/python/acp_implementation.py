...
class CoordinationManager:
    ...
    async def create_coordination(
        ...
        try:
            ...
            logger.info(f"[+] Coordination created: {coordination.coordination_id} ({coordination_type})")
    ...
    async def join_coordination(
        ...
        try:
            ...
            await self._emit_event("participant_joined", {
                "coordination_id": coordination_id,
                "participant": participant
            })
    ...
    async def leave_coordination(self, coordination_id: str, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._emit_event("participant_left", {
                "coordination_id": coordination_id,
                "agent_id": agent_id
            })
    ...
    async def update_shared_state(
        ...
        try:
            ...
            await self._emit_event("state_updated", {
                "coordination_id": coordination_id,
                "agent_id": agent_id,
                "updates": updates
            })
    ...
    async def get_progress(self, coordination_id: str) -> Dict[str, Any]:
        ...
        completed = sum(1 for task in coordination.tasks.values()
                       if task.status == TaskStatus.COMPLETED.value)
        in_progress = sum(1 for task in coordination.tasks.values()
                         if task.status == TaskStatus.IN_PROGRESS.value)
        pending = sum(1 for task in coordination.tasks.values()
                     if task.status == TaskStatus.PENDING.value)
    ...
    async def _get_coordination_summary(self, coordination: ACPCoordination) -> Dict[str, Any]:
        ...
        total_tasks = len(coordination.tasks)
        completed_tasks = sum(1 for t in coordination.tasks.values()
        ...
        return {
            ...
            "duration": self._calculate_duration(coordination.created_at, coordination.completed_at)
        }
...
class ACPClient:
    ...
    async def create_task(
        ...
        result = await self.manager.create_task(
            coordination_id, task_type, description, priority
        )
    ...
    async def update_task(
        ...
        result = await self.manager.update_task_status(
            coordination_id, task_id, status, output
        )
        return result.get("success", False)
    ...
    async def update_state(
        ...
        """Update shared state"""
        result = await self.manager.update_shared_state(
            coordination_id, self.agent_id, updates
        )
        return result.get("success", False)
...
async def example_usage():
    ...
    await worker1_client.update_task(
        ...
        output={"result": "chunk1_processed"}
    )
    ...
    await worker2_client.update_task(
        ...
        output={"result": "chunk2_processed"}
    )
